...
const ScenarioCard: React.FC<{ scenario: ComplaintScenario; onSelect: () => void; }> = ({ scenario, onSelect }) => (
    <div
        onClick={onSelect}
        className="bg-slate-800/50 border border-slate-700 rounded-lg p-6 cursor-pointer hover:bg-slate-800 hover:border-fuchsia-500 transition-all transform hover:-translate-y-1 flex flex-col h-full"
    >
        <div className="flex justify-center items-center mb-4 text-fuchsia-400">{scenario.icon}</div>
        ...
        <div className="mt-4 pt-4 border-t border-slate-700/50 text-center text-sm space-y-2">
            <p className="text-slate-400"><span className="font-semibold text-fuchsia-400 block">Your Role:</span> {scenario.userRole}</p>
            <p className="text-slate-400"><span className="font-semibold text-fuchsia-400 block">AI's Role:</span> {scenario.aiRole}</p>
        </div>
    </div>
);
            ...
            <div className="max-w-2xl mx-auto animate-fade-in">
                <div className="bg-slate-800/50 border border-slate-700 rounded-lg p-6">
                    ...
                    <textarea
                        ...
                        className="w-full h-32 p-3 bg-slate-900 border border-slate-600 rounded-md focus:ring-2 focus:ring-fuchsia-500 focus:border-fuchsia-500 transition"
                        disabled={isLoading}
                    />
                    ...
                    <div className="mt-4 flex justify-center">
                        <button onClick={handleGenerateScript} disabled={isLoading || !!generatedData} className="bg-fuchsia-600 text-white font-bold py-2 px-6 rounded-lg hover:bg-fuchsia-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
                    ...
                    {generatedData && (
                        <div className="mt-6 p-4 bg-slate-900/50 rounded-lg border border-slate-600 animate-fade-in">
                             <h3 className="text-lg font-semibold text-fuchsia-400 mb-2">Suggested Starter Script:</h3>
                             ...
                             <blockquote className="border-l-4 border-fuchsia-500 pl-4 py-2 bg-slate-800 rounded-r-lg">
                 ...
                 <div className="text-center mt-4">
                    <button onClick={resetCustomizer} className="text-sm text-slate-500 hover:text-fuchsia-400">
                        Back to Scenarios
                    </button>
                </div>
            </div>
        ...
        <div className="max-w-6xl mx-auto">
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
                ...
                <div
                    ...
                    className="bg-slate-800/50 border-2 border-dashed border-slate-600 rounded-lg p-6 cursor-pointer hover:border-fuchsia-500 hover:bg-slate-800 transition-all flex flex-col justify-center items-center h-full text-center"
                >
