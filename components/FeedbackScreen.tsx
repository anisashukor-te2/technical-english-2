...
const ScoreCircle: React.FC<{ score: number }> = ({ score }) => {
    ...
    return (
        <div className="relative flex items-center justify-center w-28 h-28">
            ...
            <span className={`absolute text-3xl font-bold ${colorClass.replace('text-green-500', 'text-green-400').replace('text-yellow-500', 'text-yellow-400').replace('text-red-500', 'text-red-400')}`}>{score}</span>
        </div>
    );
};
          ...
          <p key={sentenceIndex}>
            {parts.map((part, partIndex) =>
              ...
              keywordRegex && partIndex % 2 === 1 ? (
                <span key={partIndex} className="bg-green-800/50 text-green-300 font-semibold px-1 py-0.5 rounded">
                  {part}
                </span>
              ) : (
...
const SlidesViewer: React.FC<{ slides: Slide[] }> = ({ slides }) => {
    ...
    if (!slides || slides.length === 0) {
        return <p className="text-white">No slides were provided for this session.</p>;
    }
    ...
    return (
        <div className="flex flex-col md:flex-row gap-4 h-[40vh] max-h-[500px]">
            ...
            <div className="w-full md:w-1/3 md:border-r md:border-slate-700 md:pr-4 overflow-y-auto">
                <h4 className="text-md font-semibold text-white mb-3 sticky top-0 bg-slate-800/60 pb-2">Slide Deck</h4>
                <ul className="space-y-2">
                    {slides.map((slide, index) => (
                        <li 
                            key={slide.id} 
                            onClick={() => setCurrentSlideIndex(index)}
                            className={`p-3 rounded-lg cursor-pointer border-2 transition-colors ${currentSlideIndex === index ? 'border-cyan-700 bg-cyan-950/20' : 'border-transparent bg-slate-700/50 hover:bg-slate-700'}`}
                        >
                            <p className="text-sm font-semibold text-white truncate">{slide.title || `Slide ${index + 1}`}</p>
                            <p className="text-xs text-slate-200">Slide {index + 1}</p>
                        </li>
                    ))}
                </ul>
            </div>
            ...
            <div className="w-full md:w-2/3 overflow-y-auto">
                {currentSlide ? (
                    <div>
                        <h4 className="text-lg font-bold text-cyan-600 mb-2 sticky top-0 bg-slate-800/60 pb-2">{currentSlide.title}</h4>
                        <div className="text-white whitespace-pre-wrap text-sm leading-relaxed">
                            <p>{currentSlide.script || <span className="italic text-white">No script provided for this slide.</span>}</p>
                        </div>
                    </div>
                ) : (
                    <div className="flex items-center justify-center h-full">
                        <p className="text-white">Select a slide to view its content.</p>
                    </div>
                )}
            </div>
        </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
                 <Card title="Overall Score">
                    <div className="flex flex-col items-center justify-center p-2">
                        ...
                        <p className="text-center text-white text-sm mt-2">An aggregated score based on pacing, keyword usage, language, and clarity.</p>
                    </div>
                </Card>
                <Card title="Pacing">
                    <div className="flex flex-col items-center justify-center p-3 h-full">
                        <p className="text-5xl font-bold text-cyan-600">{feedback.pacing.wpm}<span className="text-xl font-normal text-white"> WPM</span></p>
                        <p className="mt-1 text-center text-white text-sm">{feedback.pacing.feedback}</p>
                        <p className="text-xs text-white mt-1">(Aim for 140-160 WPM)</p>
                    </div>
                </Card>
                        ...
                        {feedback.fillerWords.length > 0 ? (
                            <ul className="space-y-2">
                                {feedback.fillerWords.map(fw => (
                                    <li key={fw.word} className="flex justify-between items-center text-white">
                                        <span className="font-mono bg-slate-700 px-2 py-1 rounded text-cyan-600 text-sm">"{fw.word}"</span>
                                        <span className="font-semibold">{fw.count} time(s)</span>
                                    </li>
                                ))}
                            </ul>
                        ) : (
                            <p className="text-center text-white">No filler words detected. Great job!</p>
                        )}
                    </div>
                </Card>
            </div>
            ...
            <Card title="Recording & Transcription">
                <div className="p-4">
                   {mediaSrc && (sessionData?.recordingMimeType?.startsWith('video/') || (slides && slides.length > 0)) ? (
                        ...
                        <div className="w-full h-14 flex items-center justify-center bg-slate-700 rounded-md mb-3">
                            <p className="text-white text-sm">Loading recording...</p>
                        </div>
                    )}
                    <div className="max-h-96 overflow-y-auto text-white leading-relaxed border-t border-slate-700 pt-3 text-sm">
            ...
            <div className="grid grid-cols-1 gap-6">
                <Card title="Delivery & Content Analysis">
                    <div className="p-4 space-y-6">
                        <div>
                            <h4 className="font-semibold text-cyan-600 mb-2 text-md">Technical Keyword Usage</h4>
                            <div className="space-y-2">
                                <div>
                                    <h5 className="font-semibold text-green-400 mb-1 text-sm">Found ({feedback.keywordAnalysis.keywordsFound.length})</h5>
                                    <div className="flex flex-wrap gap-1">
                                        {feedback.keywordAnalysis.keywordsFound.length > 0 ? feedback.keywordAnalysis.keywordsFound.map(k => <span key={k} className="bg-green-900/50 text-green-300 text-xs font-medium px-2 py-0.5 rounded">{k}</span>) : <p className="text-xs text-slate-200 italic">None found.</p>}
                                    </div>
                                </div>
                                <div>
                                    <h5 className="font-semibold text-yellow-400 mb-1 text-sm">Missed ({feedback.keywordAnalysis.keywordsMissed.length})</h5>
                                    <div className="flex flex-wrap gap-1">
                                        {feedback.keywordAnalysis.keywordsMissed.length > 0 ? feedback.keywordAnalysis.keywordsMissed.map(k => <span key={k} className="bg-yellow-900/50 text-yellow-300 text-xs font-medium px-2 py-0.5 rounded">{k}</span>) : <p className="text-xs text-slate-200 italic">None missed. Well done!</p>}
                                    </div>
                                </div>
                                <p className="text-xs text-slate-200 pt-2 border-t border-slate-700/50">{feedback.keywordAnalysis.feedback}</p>
                            </div>
                        </div>

                        <div>
                            <h4 className="font-semibold text-cyan-600 mb-2 text-md">Language & Clarity</h4>
                            <p className="text-sm text-white leading-relaxed">{feedback.languageFeedback}</p>
                        </div>

                        <div>
                            <h4 className="font-semibold text-cyan-600 mb-2 text-md">Voice Modulation</h4>
                            <p className="text-sm text-white leading-relaxed">{feedback.voiceModulation.feedback}</p>
                        </div>
                    </div>
                </Card>
                ...
                {sessionData?.grade !== undefined && sessionData?.lecturerFeedback && !isLecturerView && (
                    <Card title="Lecturer's Feedback">
                        <div className="p-4 space-y-4">
                            <div>
                                <h4 className="font-semibold text-cyan-600 mb-1">Grade</h4>
                                <p className="text-2xl font-bold text-white">{sessionData.grade}%</p>
                            </div>
                            <div>
                                <h4 className="font-semibold text-cyan-600 mb-1">Comments</h4>
                                <blockquote className="p-3 bg-slate-700/50 border-l-4 border-cyan-700 text-white italic text-sm">
                    ...
                    {!isLecturerView ? (
                             ...
                             {sessionId && (
                                <div className="p-4">
                                    ...
                                    <textarea
                                        ...
                                        className="w-full p-2 bg-slate-900 border border-slate-600 rounded-md focus:ring-2 focus:ring-cyan-700 focus:border-cyan-700 transition text-sm text-white"
                                        rows={2}
                                    ></textarea>
                                    <button
                            ...
                            <div className="p-4 border-t border-slate-700">
                                <div className="flex flex-wrap items-center justify-center gap-4">
                                    <button
                                        ...
                                        className="bg-cyan-800 text-white font-bold py-2 px-4 rounded-lg hover:bg-cyan-900 transition-colors focus:outline-none focus:ring-4 focus:ring-cyan-700/50 text-sm"
                                    >
                                        Try Again
                                    </button>
                                    <button
                                    ...
                                    {sessionId && (
                                        <>
                                            <button
                                                ...
                                                className="bg-blue-800 text-white font-bold py-2 px-4 rounded-lg hover:bg-blue-900 transition-colors focus:outline-none focus:ring-4 focus:ring-blue-700/50 text-sm disabled:bg-slate-500 disabled:cursor-not-allowed"
                                            >
                        ...
                        <div className="p-4">
                            {selfReflection ? (
                                ...
                                <p className="text-sm text-white text-center">The student did not add any self-reflection notes.</p>
                            )}
                        </div>
                    )}
                </Card>
                ...
                {isLecturerView && (
                    <Card title="Lecturer Assessment">
                        <div className="p-4 space-y-4">
                            <div>
                                <label htmlFor="grade" className="block text-sm font-medium text-slate-100">Grade (%)</label>
                                <input
                                    type="number"
                                    id="grade"
                                    value={grade}
                                    ...
                                    className="mt-1 w-full p-2 bg-slate-900 border border-slate-600 rounded-md focus:ring-2 focus:ring-cyan-700 text-white"
                                />
                            </div>
                            <div>
                                <label htmlFor="lecturerFeedback" className="block text-sm font-medium text-slate-100">Written Feedback</label>
                                <textarea
                                    id="lecturerFeedback"
