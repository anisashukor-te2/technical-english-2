    ...
    return (
        <div className="flex space-x-1">
            {[1, 2, 3, 4, 5].map((star) => (
                ...
                >
                    <svg
                        className={`w-6 h-6 ${rating >= star ? 'text-yellow-400' : 'text-slate-600'}`}
                        fill="currentColor"
                        viewBox="0 0 20 20"
                    >
                     ...
                     <Card title="Recording & Transcription">
                        <div className="p-4">
                            {mediaSrc ? (
                                ...
                                <div className="w-full h-14 flex items-center justify-center bg-slate-900 rounded-md mb-4">
            ...
            {isLoading ? <p>Loading sessions...</p> : sessions.length === 0 ? (
                ...
                <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {sessions.map(session => (
                        <div key={session.id} className="bg-slate-800/50 border border-slate-700 rounded-lg p-4 flex flex-col justify-between">
                            <div>
                                <p className="text-sm font-semibold text-slate-300">Anonymous Session</p>
