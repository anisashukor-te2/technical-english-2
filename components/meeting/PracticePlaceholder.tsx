...
const ScenarioCard: React.FC<{ scenario: MeetingScenario; onSelect: () => void; }> = ({ scenario, onSelect }) => (
    ...
    >
        ...
        <div className="mt-4 pt-4 border-t border-slate-200 text-left text-sm space-y-2">
            <p className="text-slate-600"><span className="font-semibold text-blue-600 block">Available Roles:</span> {scenario.roles.join(', ')}</p>
        </div>
    </div>
);
    ...
    return (
        <div className="max-w-5xl mx-auto">
            <div className="text-center mb-8">
                <h2 className="text-3xl font-bold text-slate-900">Practice: Meeting Simulations</h2>
                <p className="mt-2 text-lg text-slate-600">Select a scenario to start your practice session.</p>
            </div>
