...
const ScenarioCard: React.FC<{ scenario: MeetingScenario; onSelect: () => void; }> = ({ scenario, onSelect }) => (
    <div
        onClick={onSelect}
        className="bg-slate-800/60 border border-slate-700 rounded-lg p-6 cursor-pointer hover:bg-slate-700/80 hover:border-cyan-400 transition-all transform hover:-translate-y-1 flex flex-col h-full shadow-lg"
    >
        <div className="flex justify-center items-center mb-4 text-cyan-400">{scenario.icon}</div>
        <h3 className="text-xl font-bold text-white text-center">{scenario.title}</h3>
        <p className="mt-2 text-slate-300 flex-grow text-center">{scenario.description}</p>
        <div className="mt-4 pt-4 border-t border-slate-700 text-left text-sm space-y-2">
    ...
    return (
        <div className="max-w-5xl mx-auto">
            <div className="text-center mb-8">
                ...
                <p className="mt-2 text-lg text-slate-300">Select a scenario to start your practice session.</p>
            </div>
