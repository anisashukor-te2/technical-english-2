...
export const PresentationReviewScreen: React.FC<PresentationReviewScreenProps> = ({ user, userType, selectedClass }) => {
  ...
  const fetchSessions = async () => {
    ...
    try {
        ...
        setError(error.message || "Failed to load your history. Please check your connection and try again.");
    } finally {
        setIsLoading(false);
    }
  };
            ...
            <Card title="Completed Sessions">
                {isLoading ? (
                    ...
                    <div className="p-8 text-center">
                        ...
                        <button onClick={fetchSessions} className="bg-slate-700 hover:bg-slate-600 text-white px-4 py-2 rounded-lg transition-colors">Retry</button>
                    </div>
                ) : sessions.length === 0 ? (
                    <div className="p-8 text-center border-dashed border-2 border-slate-700 rounded-lg m-4">
                    ...
                    {sessions.map(session => (
                        <div key={session.id} className={`p-3 bg-slate-900/50 rounded-lg flex items-center justify-between ${session.isSubmitted && !session.grade ? 'animate-highlight-fade border-l-4 border-blue-600' : 'border-l-4 border-transparent'}`}>
                            ...
                            <button onClick={() => setSelectedSession(session)} className="text-sm bg-slate-700 hover:bg-cyan-800 px-3 py-1 rounded text-slate-200 transition-colors">View Details</button>
                        </div>
                    ))}
                    </div>
                )}
  ...
  return (
    <div className="max-w-5xl mx-auto animate-fade-in pb-24">
        ...
        {/* Stats Cards - Render even if 0 */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
            <StatCard label="Average Score" value={studentSessions.length > 0 ? `${stats.avgScore}%` : '-'} icon={<svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>} />
            <StatCard label="Average Pace" value={studentSessions.length > 0 ? `${stats.avgWpm} WPM` : '-'} icon={<svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 10V3L4 14h7v7l9-11h-7z" /></svg>} />
        ...
        {!isLoading && !error && studentSessions.length > 0 && (
        ...
        {/* Practice History - With Error/Empty Handling INSIDE */}
        <div className="mt-6">
            <Card title="Practice History">
                 {isLoading ? (
                    ...
                    <div className="p-8 text-center">
                        ...
                        <button onClick={fetchSessions} className="bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded-lg transition-colors">Retry</button>
                    </div>
                ) : studentSessions.length === 0 ? (
                    <div className="text-center p-8">
                         <svg xmlns="http://www.w3.org/2000/svg" className="mx-auto h-12 w-12 text-slate-600" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M3 13.125C3 12.504 3.504 12 4.125 12h2.25c.621 0 1.125.504 1.125 1.125v6.75C7.5 20.496 6.996 21 6.375 21h-2.25A1.125 1.125 0 013 19.875v-6.75zM9.75 8.625c0-.621.504-1.125 1.125-1.125h2.25c.621 0 1.125.504 1.125 1.125v11.25c0 .621-.504 1.125-1.125 1.125h-2.25a1.125 1.125 0 01-1.125-1.125V8.625zM16.5 4.125c0-.621.504-1.125 1.125-1.125h2.25C20.496 3 21 3.504 21 4.125v15.75c0 .621-.504 1.125-1.125 1.125h-2.25a1.125 1.125 0 01-1.125-1.125V4.125z" /></svg>
                        <p className="text-slate-500 mt-2">No sessions yet. Start practicing to see your history here.</p>
                    </div>
                ) : (
        ...
        {/* Badges - Always rendered for Student */}
        <div className="mt-6">
        ...
        <div className="text-center mt-8">
            <button onClick={handleBackToSelection} className="bg-slate-600 text-white font-bold py-2 px-6 rounded-lg hover:bg-slate-500 transition-colors">
                Back to Menu Selection
            </button>
        </div>
        {selectedSession && (
